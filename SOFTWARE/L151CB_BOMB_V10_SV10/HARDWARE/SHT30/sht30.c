#include "sht30.h"
#include "delay.h"

#define  i2cAddWrite_8bit           0x88
#define  i2cAddRead_8bit            0x89
/*CRC*/
#define POLYNOMIAL             0x131   // P(x) = x^8 + x^5 + x^4 + 1 = 100110001

float Tem_Value = 0.0;
float  RH_Value = 0.0;

u8 buffer[6];

void SHT3X_WriteCMD(u16 cmd)
{
	SHT_IIC_Start();
	SHT_IIC_Send_Byte(i2cAddWrite_8bit);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(cmd>>8);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(cmd);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Stop();
	delay_ms(10);
}

void SHT3X_SetPeriodicMeasurement()
{
    SHT3X_WriteCMD(CMD_MEAS_PERI_2_H);
}

void SHT3X_ReadState(u8 *temp)
{
    SHT_IIC_Start();
	SHT_IIC_Send_Byte(i2cAddWrite_8bit);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(0xf3);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(0X2d);
	SHT_IIC_Wait_Ack();
//     SHT_IIC_Stop(); 
	delay_ms(100);
    SHT_IIC_Start();
	SHT_IIC_Send_Byte(i2cAddRead_8bit);
	SHT_IIC_Wait_Ack();

	temp[0] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	temp[1] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	temp[2] = SHT_IIC_Read_Byte(0);
//	SHT_IIC_NAck(); 
	SHT_IIC_Stop(); 
}

void SHX3X_ReadResults(u16 cmd,  u8 *p)
{
	SHT_IIC_Start();
	SHT_IIC_Send_Byte(i2cAddWrite_8bit);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(cmd>>8);
	SHT_IIC_Wait_Ack();
	SHT_IIC_Send_Byte(cmd);
	SHT_IIC_Wait_Ack();
//	SHT_IIC_Stop();
	
	delay_ms(10);
	SHT_IIC_Start();
	SHT_IIC_Send_Byte(i2cAddRead_8bit);
	SHT_IIC_Wait_Ack();

	p[0] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	p[1] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	p[2] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	p[3] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	p[4] = SHT_IIC_Read_Byte(1);
//	SHT_IIC_Ack();
	p[5] =SHT_IIC_Read_Byte(0);
//	SHT_IIC_NAck();
	SHT_IIC_Stop();
	delay_ms(10);
}


u8 IC_Read_Byte(void)
{
	unsigned char i,receive=0;
	SHT_SDA_IN();//SDA设置为输入
    for(i=0;i<8;i++ )
	{
        SHT_IIC_SCL=0; 
        delay_us(2);
		SHT_IIC_SCL=1;
        receive<<=1;
        if(SHT_READ_SDA)receive++;   
		delay_us(1); 
    }			
return receive;	
}

void SHT_Init(void)
{
//	GPIO_InitTypeDef GPIO_InitStructure;
////	RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOB, ENABLE );	
//    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE);	 //使能PB端口时钟
//	   
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;
//    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT; 		 //推挽输出
//    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
//	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
//	GPIO_Init(GPIOB, &GPIO_InitStructure);
//    
//	SHT_IIC_SDA=1;	  	  
//	SHT_IIC_SCL=1;	
	
	
    delay_ms(200); /* Must add delay */
    SHT3X_SetPeriodicMeasurement();
    delay_ms(200); /* Must add delay */
}

u8 SHT3X_CalcCrc(uint8_t *data, uint8_t nbrOfBytes)
{
	uint8_t bit;        // bit mask
    uint8_t crc = 0xFF; // calculated checksum
    uint8_t byteCtr;    // byte counter

    // calculates 8-Bit checksum with given polynomial
    for(byteCtr = 0; byteCtr < nbrOfBytes; byteCtr++) {
        crc ^= (data[byteCtr]);
        for(bit = 8; bit > 0; --bit) {
            if(crc & 0x80) {
                crc = (crc << 1) ^ POLYNOMIAL;
            }  else {
                crc = (crc << 1);
            }
        }
    }
	return crc;
}

u8 SHT3X_CheckCrc(uint8_t *pdata, uint8_t nbrOfBytes, uint8_t checksum)
{
    uint8_t crc;
	crc = SHT3X_CalcCrc(pdata, nbrOfBytes);// calculates 8-Bit checksum
    if(crc != checksum) 
    {   
        return 1;           
    }
    return 0;              
}

float SHT3X_CalcTemperature(uint16_t rawValue)
{
    // calculate temperature 
    float temp;
    temp = (175 * (float)rawValue / 65535 - 45) ; // T = -45 + 175 * rawValue / (2^16-1)
    return temp;
}

float SHT3X_CalcRH(uint16_t rawValue)
{
    // calculate relative humidity [%RH]
    float temp1 = (100 * (float)rawValue / 65535) ;  // RH = rawValue / (2^16-1) * 100

    return temp1;
}

void SHT_GetValue(void)
{	
    u8 temp = 0;
    u16 dat;
    u8 p[3];
    
    SHX3X_ReadResults(CMD_FETCH_DATA, buffer);
    /* check tem */
    p[0] = buffer[0];
    p[1] = buffer[1];
    p[2] = buffer[2];
    temp = SHT3X_CheckCrc(p,2,p[2]);
   if( !temp ) /* value is ture */
    {
        dat = ((u16)buffer[0] << 8) | buffer[1];
        Tem_Value = SHT3X_CalcTemperature( dat );    
    }
    /* check humidity */
    p[0] = buffer[3];
    p[1] = buffer[4];
    p[2] = buffer[5];
    temp = SHT3X_CheckCrc(p,2,p[2]);
    if( !temp )
    {
        dat = ((u16)p[0] << 8) | p[1];
        RH_Value = SHT3X_CalcRH( dat ); 
    }
}

/*******************  以下为sht30的SHT_IIC初始化    *******************/

//初始化SHT_IIC
void SHT_IIC_Init(void)
{					     
	GPIO_InitTypeDef GPIO_InitStructure;
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE );	//使能GPIOB时钟
	   
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT ;   //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_40MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_SetBits(GPIOB,GPIO_Pin_6 | GPIO_Pin_7); 	//PB6,PB7 输出高
	
}
//产生SHT_IIC起始信号
void SHT_IIC_Start(void)
{
	SHT_SDA_OUT();     //sda线输出
	SHT_IIC_SDA=1;	  	  
	SHT_IIC_SCL=1;
	delay_us(4);
 	SHT_IIC_SDA=0;//START:when CLK is high,DATA change form high to low 
	delay_us(4);
	SHT_IIC_SCL=0;//钳住I2C总线，准备发送或接收数据 
}	  
//产生SHT_IIC停止信号
void SHT_IIC_Stop(void)
{
	SHT_SDA_OUT();//sda线输出
	SHT_IIC_SCL=0;
	SHT_IIC_SDA=0;//STOP:when CLK is high DATA change form low to high
 	delay_us(4);
	SHT_IIC_SCL=1; 
	SHT_IIC_SDA=1;//发送I2C总线结束信号
	delay_us(4);							   	
}
//等待应答信号到来
//返回值：1，接收应答失败
//        0，接收应答成功
u8 SHT_IIC_Wait_Ack(void)
{
	u8 ucErrTime=0;
	SHT_SDA_IN();      //SDA设置为输入  
	SHT_IIC_SDA=1;delay_us(1);	   
	SHT_IIC_SCL=1;delay_us(1);	 
	while(SHT_READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
		{
//			SHT_IIC_Stop();
			SHT_IIC_SCL=0;
			return 1;
		}
	}
	SHT_IIC_SCL=0;//时钟输出0 	   
	return 0;  
} 
//产生ACK应答
void SHT_IIC_Ack(void)
{
	SHT_IIC_SCL=0;
	SHT_SDA_OUT();
	SHT_IIC_SDA=0;
	delay_us(2);
	SHT_IIC_SCL=1;
	delay_us(2);
	SHT_IIC_SCL=0;
}
//不产生ACK应答		    
void SHT_IIC_NAck(void)
{
	SHT_IIC_SCL=0;
	SHT_SDA_OUT();
	SHT_IIC_SDA=1;
	delay_us(2);
	SHT_IIC_SCL=1;
	delay_us(2);
	SHT_IIC_SCL=0;
}					 				     
//SHT_IIC发送一个字节
//返回从机有无应答
//1，有应答
//0，无应答			  
void SHT_IIC_Send_Byte(u8 txd)
{                        
    u8 t;   
	SHT_SDA_OUT(); 	    
    SHT_IIC_SCL=0;//拉低时钟开始数据传输
    for(t=0;t<8;t++)
    {              
        //SHT_IIC_SDA=(txd&0x80)>>7;
		if((txd&0x80)>>7)
			SHT_IIC_SDA=1;
		else
			SHT_IIC_SDA=0;
		txd<<=1; 	  
		delay_us(2);   //对TEA5767这三个延时都是必须的
		SHT_IIC_SCL=1;
		delay_us(2); 
		SHT_IIC_SCL=0;	
		delay_us(2);
    }	 
} 	    
//读1个字节，ack=1时，发送ACK，ack=0，发送nACK   
u8 SHT_IIC_Read_Byte(unsigned char ack)
{
	unsigned char i,receive=0;
	SHT_SDA_IN();//SDA设置为输入
    for(i=0;i<8;i++ )
	{
        SHT_IIC_SCL=0; 
        delay_us(2);
		SHT_IIC_SCL=1;
        receive<<=1;
        if(SHT_READ_SDA)receive++;   
		delay_us(1); 
    }					 
    if (!ack)
        SHT_IIC_NAck();//发送nACK
    else
        SHT_IIC_Ack(); //发送ACK   
    return receive;
}




